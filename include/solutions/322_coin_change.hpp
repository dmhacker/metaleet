...
namespace detail322 {
    ...
    struct solve_impl {
        ...
        struct coin_loop_impl<change, coin, metal::number<-1>> {
            using type = change;
        };

        template <class change, class coin>
        using coin_loop = typename coin_loop_impl<change, coin,
            typename solve_impl<coins, metal::sub<amount, coin>, metal::less<amount, coin>>::type>::type;

        using type = metal::accumulate<metal::lambda<coin_loop>, metal::number<-1>, coins>;
    };

    template <class coins, class is_neg_amount>
    struct solve_impl<coins, ZERO, is_neg_amount> {
        using type = ZERO;
    };

    template <class coins, class amount>
    struct solve_impl<coins, amount, metal::true_> {
...
template <class coins, class amount>
using solve322 = typename detail322::solve_impl<coins, amount, metal::less<amount, ZERO>>::type;
